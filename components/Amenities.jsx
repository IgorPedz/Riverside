...
export default function Amenities() {
  ...
  const handleSwipe = () => {
    ...
    if (delta > 50) {
      ...
      setSelectedIndex(
        (prev) => (prev - 1 + udogodnienia.length) % udogodnienia.length
      );
    }
  };
  ...
  return (
    <section className="py-16 bg-gray-100">
      ...
      <motion.div
        ...
        className="hidden max-w-6xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden mb-12 md:flex flex-row items-center"
      ...
      >
        ...
        <div className="p-6 md:w-1/2 text-center md:text-left">
          <h3 className="text-2xl font-bold mb-2">
            {udogodnienia[selectedIndex].name}
          </h3>
          <p className="text-gray-700">
            {udogodnienia[selectedIndex].description}
          </p>
        </div>
      </motion.div>
